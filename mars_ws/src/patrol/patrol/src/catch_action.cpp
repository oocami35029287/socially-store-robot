...
public:
    ...
    void dope_callback(visualization_msgs::MarkerArray msg);
    ...
    visualization_msgs::MarkerArray dope_list;
};
...
{  
    ...
    dope_sub = nh.subscribe("/dope/markers", 1, &warehouse_action::dope_callback, this);
...
void warehouse_action::dope_callback(visualization_msgs::MarkerArray msg)
{
    if(msg.markers.size() != 0 && reach){
...
void warehouse_action::Position_Manager()
{
    ...
    while(1)
    {
        ...
        if(command == 'g'){
            ...
            if(dope_list.markers.empty()){
                cout<<"No DOPE object found."<<endl;
                goto lab_end;
            }
            for(visualization_msgs::Marker &itr:dope_list.markers){
                //std::string dope_str = itr.text.substr(0,itr.text.find('('));
                //cout<<dope_str<<endl;
                if(!strcmp(itr.text.substr(0,text.length()).c_str(),text.substr(0,text.length()).c_str())){ 
                    //success compare;
                    object_pose = itr.pose;
                    cout<<"get!"<<endl;
                    check = true; 
                    continue;
                }
