...
void PathFindingNode::finalgoal_cb(const geometry_msgs::PoseStamped::ConstPtr &goal_msg_ptr) {
  ...
  //goal_msg_ptr from map to odom
  if (goal_msg_ptr) {
    try {
        // 定义tf2变换
        tf::StampedTransform tf_odom2map;

        // 使用tf2库来获取从map到odom的变换
        tflistener_.waitForTransform("odom", "map", ros::Time(0),ros::Duration(subgoal_timer_interval_ ));
        tflistener_.lookupTransform("odom", "map", ros::Time(0), tf_odom2map);
        printf("transform %f %f ",tf_odom2map.getOrigin ().getX(),tf_odom2map.getOrigin ().getY());
        geometry_msgs::TransformStamped tf_odom2map_msg;
        tf::transformStampedTFToMsg(tf_odom2map, tf_odom2map_msg);
        // 使用tf2库来执行坐标系转换
        geometry_msgs::PoseStamped goal_in_odom;
        tf2::doTransform(*goal_msg_ptr, goal_in_odom, tf_odom2map_msg);
        printf("pose %f %f ",goal_in_odom.pose.position.x,goal_in_odom.pose.position.y);

        //将转换后的消息存储在finalgoal_ptr_
        finalgoal_ptr_.reset(new geometry_msgs::PoseStamped(goal_in_odom));
    } catch (tf2::TransformException& ex) {
        ROS_ERROR("Transform failed: %s", ex.what());
        publish_robot_status_marker("no transform.");
    }
  } else {
      finalgoal_ptr_.reset(); // 将 finalgoal_ptr_ 设置为空
      publish_robot_status_marker("no goal in .");
  }
  ...
  if(localmap_ptr_ && finalgoal_ptr_ && is_footprint_safe(localmap_ptr_, footprint_ptr_)) {
    geometry_msgs::Point subgoal_pt = generate_subgoal(localmap_ptr_, finalgoal_ptr_, tf_base2odom);
    // A* path planning
